...
function statusBadgeEl(a) {
	if (a.type === 'arrow' && !(a.mark && a.mark.kind)) return null; // 무마킹 화살표 — 유령 '신규' 배지 제거(마킹해야 배지 노출)
...
function renderAnnotPanel() {
	...
	// 도구 팔레트 — 편집 모드에서만. 진입 시 현재 도구를 새 오버레이에 재적용.
	if (apTools) apTools.classList.toggle('hidden', !tab.editMode);
	if (tab.editMode && tab.overlay) tab.overlay.setTool(activeTool);
...
function renderDetail() {
	...
	const isTextLike = isText || isArrow; // 텍스트·화살표는 묶음·슬롯 없음 — 자유 편집기(내용/메모)만.
	// 마킹 블록 — 화살표는 신규/기존 마킹 노출(색 반영), 텍스트만 숨김. 묶음·슬롯은 둘 다 숨김.
	...
	if (isArrow) renderMark(ann); // 화살표 마킹 렌더(신규/기존 → 선·화살촉 색). applyMark → overlay.refresh 로 색 즉시 반영.
	if (isTextLike) {
		apdSlotMode = false;
		apdSlots.classList.add('hidden');
		apdEditor.classList.remove('hidden');
		apdEditor.setAttribute('data-placeholder', isArrow ? '화살표 메모(선택)…' : '텍스트 내용을 입력하세요…');
		if (document.activeElement !== apdEditor) apdEditor.innerHTML = (ann.body && ann.body.html) || '';
		return;
	}
	apdEditor.setAttribute('data-placeholder', '핀 설명을 입력하세요…');
	renderMark(ann);
...
if (apdEditor) {
	apdEditor.addEventListener('input', () => {
		...
		markDirty(tab);
		updateRowText(ann);
		if (ann.type === 'text' && tab.overlay) tab.overlay.refresh(); // 캔버스 텍스트 내용 즉시 반영
	});
}
